...
export function PostItem({ id, content, createdAt, isNeighbor, isOwn }: PostItemProps) {
  ...
  return (
    <li className="glass rounded-2xl p-4">
      <p className="whitespace-pre-wrap break-words text-[15px] leading-relaxed text-stone-100">{content}</p>
      <div className="mt-2.5 flex items-center gap-2 text-[11px] text-stone-500">
        ...
        <span className="ml-auto flex gap-3">
          {isOwn && (
            <button onClick={handleDelete} className="text-stone-500">
              {t('delete')}
            </button>
          )}
          {done ? (
            ...
            <button onClick={() => setReporting(!reporting)} className="text-stone-500">
              {t('report')}
            </button>
          )}
        </span>
      ...
      {reporting && (
        <div className="mt-3 space-y-2 border-t border-white/10 pt-3">
          <p className="text-xs font-semibold text-stone-300">{t('reportTitle')}</p>
          <select
            value={reason}
          ...
          <input
            ...
            maxLength={200}
            className="w-full rounded-xl bg-white/10 px-3 py-2 text-sm text-stone-100 outline-none placeholder:text-stone-500 focus:ring-2 focus:ring-accent/40"
          />
          {reportError && <p className="text-xs text-accent">{reportError}</p>}
          <div className="flex gap-2">
            <button
              onClick={handleReport}
              className="rounded-full bg-stone-50 px-4 py-2 text-xs font-medium text-ink transition-transform duration-150 active:scale-95"
            >
              {t('reportSubmit')}
            </button>
            <button
